...
class TypingApp {
  ...
  bindNav(){
    ...
    document.getElementById('theme-toggle').addEventListener('click',()=>{
      const themes = ['dark', 'light', 'aditya'];
  ...
  initTest(){
    ...
    this.typedChars=[];this.startTime=null;this.timeLeft=this.testDuration;
  ...
  handleInput(e,mode){
    ...
    // Check completion for lessons/practice
    if(this.currentIndex>=text.length&&mode!=='test'){this.completeSession(mode);}
  }
  ...
  showResults(){
    const elapsed=this.testDuration;const correctChars=this.typedChars.filter(c=>c.correct).length;
    const totalChars=this.typedChars.length;const wpm=Math.round((correctChars/5)/(elapsed/60));
  ...
  bindSettings(){
    ...
    document.getElementById('export-data').addEventListener('click',()=>{
      ...
      const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='velocis-typing-stats.json';a.click();
    });
  }
  // Virtual Keyboard
  ...
  async saveResultToServer(wpm,raw,acc,errors,chars,duration,difficulty,mode){
    ...
    try{await fetch('/api/results',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({wpm,rawWpm:raw,accuracy:acc,errors,chars,duration,difficulty,mode})});}catch(e){}
  }
